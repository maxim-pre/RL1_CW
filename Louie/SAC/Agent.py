...
class SACAgent:
    ...
    def __init__(self,
        ...
        self.log_alpha = nn.Parameter(torch.tensor(log_alpha, requires_grad=True, device=self.device))
    ...
    def update_critic_networks(self,
        ...
        with torch.no_grad():
            ...
            q_target = mb_rewards + discount_factor * (1 - mb_dones) * (q_min_next - self.alpha * logp)
    ...
    def update_actor_network_and_alpha(self,
                                       minibatch: Tuple[torch.tensor, ...],
        ...
        actor_loss = -(q_min_pred - self.alpha.detach() * logp).mean()
    ...
    def save_run(self,
        ...
        with open("outputs/episode_alphas.pkl", "wb") as f:
            pickle.dump(episode_alphas, f)
        ...
        settings = {
            ...
            "alpha_grad_clip": alpha_grad_clip,
